...
Features:
- Macro calculation via Mifflin-St Jeor + activity level.
- LLM-generated plan via Hugging Face models (replaces Ollama).
- Dummy pricing catalog for cost estimation (no real product search).
- CORS enabled for static frontend calls.
"""
...
def _load_hf_model():
    """Load Hugging Face model once on startup"""
    global _hf_model, _hf_tokenizer
    if _hf_model is None:
        model_name = os.getenv("HF_MODEL", "google/flan-t5-large")
...
def _init_services() -> None:
    """
    Initialize RAG service and load HF model on startup.
    """
    # Initialize RAG
    try:
        rag = RagService(ROOT_DIR / "data" / "chroma")
    ...
    try:
        _load_hf_model()
        app.state.hf_model_loaded = True
    except Exception as exc:
        app.state.hf_model_loaded = False
        app.state.hf_error = str(exc)
        print(f"Failed to load HF model: {exc}")
...
def _hf_generate(text: str, model: str) -> Optional[str]:
    """Call Hugging Face model. Returns raw response text or None on error."""
    try:
        model_obj, tokenizer = _load_hf_model()
        ...
        # Remove the prompt from response if model echoes it
...
def _recipe_model_name() -> Optional[str]:
    return os.getenv("HF_RECIPE_MODEL") or os.getenv("HF_MODEL")
...
def generate_recommendations_with_llm(req: RecommendRequest) -> Optional[List[DishRecommendation]]:
    ...
    if not model:
        raise HTTPException(
            status_code=500,
            detail="No model configured. Please set HF_MODEL or HF_RECIPE_MODEL environment variable."
        )
    
    # Check if model is loaded
    if not getattr(app.state, "hf_model_loaded", False):
        error_msg = getattr(app.state, "hf_error", "Unknown error")
        raise HTTPException(
            status_code=503,
            detail=f"Model failed to load at startup: {error_msg}. Check if model exists and you have enough memory/disk space."
        )
    days = 1 if req.period == RecommendationPeriod.one_day else 7
    needed_dishes = req.meals_per_day * days
    ingredients_txt = ", ".join(req.ingredients) if req.ingredients else "none specified—use pantry-friendly staples that fit the diet"
...
Respond with JSON only, no markdown code fences or extra text. Each dish must have 3-8 short steps (one action per step).
"""
    raw = _hf_generate(prompt, model=model)
    if not raw:
        return None
    try:
        data = _parse_json_block(raw)
...
def generate_plan_with_llm(req: PlanRequest, targets: dict) -> Optional[List[DayPlan]]:
    """
    Attempt to create a plan via Hugging Face model. Expects JSON output matching the DayPlan schema.
    Returns None on any failure; caller raises.
    """
    ...
    if not model:
        raise HTTPException(
            status_code=500,
            detail="No model configured. Please set HF_MODEL environment variable."
        )
...
def generate_plan(req: PlanRequest) -> PlanResponse:
    ...
    if days is None:
        raise HTTPException(
            status_code=503,
            detail="Model returned invalid plan. The model may not support this task format.",
        )

    estimated_cost, unknown = estimate_plan_cost(days)

    notes = [
        "Plan generated by Hugging Face model (HF_MODEL).",
        "When product search is available, map each ingredient to real products and prices.",
        "Cost uses data/dummy_catalog.csv; quantities are ignored—replace with real pricing for production.",
    ]
...
def recommend(req: RecommendRequest) -> RecommendationResponse:
    ...
    if dishes is None:
        raise HTTPException(
            status_code=503,
            detail="Model returned invalid recommendations. The model may not support this task format."
        )
...
def rag_query(req: RagQueryRequest) -> dict:
    ...
    if not rag:
        raise HTTPException(status_code=503, detail="RAG service unavailable; check dependencies.")
    prompt, sources = rag.build_prompt(req.question, k=req.k)
    raw = _hf_generate(prompt, model=_recipe_model_name() or os.getenv("HF_MODEL"))
    if not raw:
        raise HTTPException(status_code=503, detail="LLM unavailable for RAG response.")
    return {"answer": raw, "sources": sources}
...
def health() -> dict:
    return {
        "status": "ok",
        "hf_model_loaded": getattr(app.state, "hf_model_loaded", False),
        "hf_error": getattr(app.state, "hf_error", None),
        "hf_model_configured": os.getenv("HF_MODEL") is not None,
        "hf_recipe_model_configured": os.getenv("HF_RECIPE_MODEL") is not None,
...
def models() -> dict:
    return {
        ...
        "hf_recipe_model": os.getenv("HF_RECIPE_MODEL"),
        "model_loaded": getattr(app.state, "hf_model_loaded", False),
        "error": getattr(app.state, "hf_error", None)
    }
